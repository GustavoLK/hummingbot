...
from hummingbot.strategy.script_strategy_base import s_decimal_nan
...
class GenericV2StrategyWithCashOut(StrategyV2Base):
    ...
    def buy(self, connector_name: str, trading_pair: str, amount: Decimal, order_type: OrderType, price=s_decimal_nan,
            position_action=PositionAction.OPEN) -> str:
        msg = f"GLK estoy en BUY !!"
        if self.dry_run:
            Notificator().notify("AVISO", msg)
            return ""
        else:
            return super().buy(connector_name, trading_pair, amount, order_type, price, position_action)

    def sell(self, connector_name: str, trading_pair: str, amount: Decimal, order_type: OrderType, price=s_decimal_nan,
             position_action=PositionAction.OPEN) -> str:
        msg = f"GLK estoy en SELL !!"
        if self.dry_run:
            Notificator().notify("AVISO", msg)
        else:
